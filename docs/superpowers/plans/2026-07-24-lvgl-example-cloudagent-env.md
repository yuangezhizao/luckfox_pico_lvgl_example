...
- **官方镜像原始预装实测**（`docker run --rm luckfoxtech/luckfox_pico:1.0 dpkg-query`，07-28）：已装 `git 1:2.34.1-1ubuntu1.10`、`cmake 3.22.1-1ubuntu1.22.04.1`、`ca-certificates`、`pkg-config 0.29.2`、`file`、`tzdata 2023c`；未装 `sudo`，未装 `curl`（exec-daemon 引导需其自装，见 spec §5.1/§5.4），未装 `build-essential` 元包（其依赖中仅缺 `dpkg-dev`，gcc/g++/make/libc6-dev/binutils 齐全）。时区已写入镜像（`/etc/localtime` 与 `/etc/timezone` 均 `Asia/Hong_Kong`），故该 Dockerfile 刻意不设 `ENV TZ`。
